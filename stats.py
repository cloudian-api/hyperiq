...
queries = {
        ...
        '# of GETs over last 1 day for bucket blah': "sum by (bucketname, cluster) (increase(hs_get_request_duration_microseconds_count{bucketname='blah'}[1d]))",
        '# of PUTs over last 1 day for bucket blah': "sum by (bucketname, cluster) (increase(hs_put_request_duration_microseconds_count{bucketname='blah'}[1d]))",
        ...
        '# of GETs in the last 4 hrs': "sum(increase(hs_s3operations{s3operation='getObject'}[4h]))",
        '# of 300/400/500s in last day': "sum(increase(hs_http_requests_total{http_status=~'(3|4|5)..'}[1d]))",
        '# of 500s/sec over last 12 hrs': "sum(irate(hs_http_requests_total{http_status=~'5..'}[12h]))"
}
